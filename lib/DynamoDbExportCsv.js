...
function DynamoDBExportCSV(awsAccessKeyId, awsSecretAccessKey, awsRegion) {
    ...
    function getItemWithoutHeaders(item) {
        var row = {};
	var flattened_item = flattenObject(item)
        _.each(item, function (value, key) {
            row[key] = flattened_item[key].S || flattened_item[key].N || flattened_item[key].BOOL || flattened_item[key];
        });
    ...
    function getItemWithHeaders(item, columns) {
        ...
        _.each(columns, function (column) {
            if (flattened_item[column]) {
                row[column] = flattened_item[column].S || flattened_item[column].N || flattened_item[column].BOOL || flattened_item[column];
            }
            else {
                row[column] = '';
            }
